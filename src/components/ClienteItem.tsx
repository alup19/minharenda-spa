...
export default function ClienteItem({ cliente, clientes, setClientes }: listaClienteProps) {
  ...
  const { register, handleSubmit, reset } = useForm<Inputs>({
    defaultValues: { nome: cliente.nome, notas: cliente.notas ?? "", endereco: (cliente as any).endereco ?? "", telefone: (cliente as any).telefone ?? "",}
  })

  async function atualizarCliente(data: Inputs) {
    const payloadAtualizado: Inputs = { nome: data.nome, endereco: data.endereco, telefone: data.telefone, notas: data.notas, usuarioId: usuario.id}
    ...
    if (response.status === 200) {
      ...
      setClientes(clientesAnteriores =>
        clientesAnteriores.map((clienteLista: any) => (clienteLista.id === cliente.id ? { ...clienteLista, ...clienteAtualizado } : clienteLista))
      )
  ...
  const totalGastoFormatado = (cliente.totalGasto ?? 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL", minimumFractionDigits: 2,})
    ...
    <section>
      <div className='flex flex-col gap-[0.44rem]'>
        <div className='bg-[#E2E2E2] py-[0.875rem] px-[1.06rem] rounded-[0.9375rem] flex flex-row justify-between items-center'>
          ...
          <p className='text-[#303030] font-inter font-semibold'>{totalGastoFormatado}</p>
          <p className='text-[#656565] font-inter font-normal'>{numeroCompras}</p>
          <button type="button" onClick={() => setOpenNotas(true)} className="relative left-9 w-[2rem]"><img src="/article_p.svg" alt="Notas" /></button>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="font-inter">
              ...
              <DropdownMenuItem
                ...
                }}>Alterar Dados</DropdownMenuItem>
        ...
        <div className="container">
          ...
          <form onSubmit={handleSubmit(atualizarCliente)} className='container flex flex-col items-center'>
            ...
            <div className="flex gap-4">
              <button type="button" onClick={() => setOpenAlterarClientes(false)} className="text-white bg-[#292727] rounded-md px-6 py-2 text-[1rem] hover:bg-[#3a3939] font-bold font-inter hover:opacity-90 transition cursor-pointer">Cancelar</button>
        ...
        <div className="container">
          ...
          <div className='container flex flex-col items-center'>
            <div className='flex flex-col items-center my-6'>
              <p className='font-inter'>Você tem certeza que deseja apagar os dados de <span className='font-inter font-bold'>{cliente.nome}</span>?</p>
              <p className='font-inter'>Após confirmar, essa ação será irreversível.</p>
            </div>

            <div className="flex gap-4">
              <button type="button" onClick={() => setOpenExcluirClientes(false)} className="text-white bg-[#292727] rounded-md px-6 py-2 text-[1rem] hover:bg-[#3a3939] font-bold font-inter hover:opacity-90 transition cursor-pointer">Cancelar</button>
